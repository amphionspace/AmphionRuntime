...
- 用户明确允许有限的 UNKNOWN 邻接补全：同一原句内，首尾只有一个相邻已知角色，或前后已知角色相同，且未知段不超过 2500 ms、未知段及相邻段无重叠/副角色、原时间线没有相反的已知角色证据时，可以补全并合并文字。必须标记 `speakerInferred=true`、将该文本段 `confidence` 置 0、保留原始 `speakerTurns`；不能跨句传播、串联推断结果继续扩散，或把补全当成声学身份精度修复。边界依据与对照见 `delivery/harmony-dingqiao/docs/UNKNOWN_SPEAKER_BACKFILL.md`。
