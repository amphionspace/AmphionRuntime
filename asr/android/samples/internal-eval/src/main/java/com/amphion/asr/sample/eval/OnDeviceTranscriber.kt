 ...
 * - onMetrics 接收并保留最后一次 utterance metrics，供 RecordingMeta 落盘
 *
 * 0.2.0 起 SDK 锁定 16 kHz：[AsrSession.acceptPcmShort] 不再接受 sampleRate 参数。
 *
 * 线程：内部 callback 在 SDK 专用线程；对外通过原子赋值暴露最新 partial / final / metrics；
...
class OnDeviceTranscriber private constructor(
    ...
    /** 最近一段 utterance 的 SDK metrics（含 e2e / rtf / nativeRssMb 等）。 */
    @Volatile
    var lastUtteranceMetrics: AmphionMetrics? = null
        private set
    ...
    private val callback = object : AsrCallback {
        ...
        override fun onMetrics(metrics: AmphionMetrics) {
            // 评测版只关心 utterance 维度的端到端延迟 / RTF / RSS；session 维度由后台脚本算
            if (metrics.kind == AmphionMetricsKind.UTTERANCE) {
                lastUtteranceMetrics = metrics
            }
        }
    }
    ...
    companion object {
         ...
         * SDK 锁定 16 kHz，无 sampleRate 参数。
         */
        fun wrap(engine: AsrEngine): OnDeviceTranscriber = OnDeviceTranscriber(engine)
    }
}
