...
class PoliceTermsBatchEvalRunner(
    ...
    fun run(
        cases: List<PoliceTermsBatchEvalCase>,
        skipIds: Set<String>,
        ...
        cases.forEachIndexed { idx, case ->
            ...
            when (outcome.kind) {
                OutcomeKind.OK -> {
                    ...
                    processed++
                    val hit = PoliceTermsTextUtil.allTermsHit(
                        case.expectedTerms,
                        outcome.text,
                    ) || PoliceTermsTextUtil.allTermsHit(case.expectedTerms, norm.text)
                    onProgress(
                        Progress(
                            idx + 1,
                            cases.size,
                            case.uttId,
                            if (hit) "term_hit" else "miss",
                        ),
                    )
                }
                OutcomeKind.EMPTY -> {
                    ...
                    evalRecorder.append(
                        ...
                        expectedTerms = case.expectedTerms,
                        asrRaw = "",
                        result = PoliceTermsNormalizeResult("", emptyList()),
                    )
    ...
    private fun decodeOne(case: PoliceTermsBatchEvalCase, gain: Float): Outcome {
        val pcm = try {
    ...
    companion object {
        private const val TAG = "PoliceTermsBatchEval"
