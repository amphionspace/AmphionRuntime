...
- 中英识别模型升级到 Police 1.4.0，上游 encoder INT8、decoder/joiner FP32；警务术语、派出所、车牌与完整指令纠错同步更新。模型及词典身份以交付包清单为准。
- 修正活动 VAD endpoint 仍有待处理语音时的收口，避免边界吞掉已接受的音频；同步保留 `isFinal`、`isLast`、`onComplete` 的既有含义。
- 优化会议角色的主要发言身份、句内可读性和短暂声学空隙处理。临时角色仍可修正，已提交窗口保持冻结；UNKNOWN 与重叠证据继续保留。
- 接入变化和验收边界见 `UPGRADE_0.3.8.md`。通用短插话、重叠身份精度及后台/锁屏可靠性仍需正式验收，不能由模型指标或单测结果推定通过。

## [0.3.7] - 2026-09-17

- Android 公开 beginTime/endTime 改为与角色 token、鸿蒙一致的毫秒四舍五入，消除浮点截断造成的负时长尾字；与旧版数值最多相差 1 ms。
- Demo 按 sourceUtteranceId 展示完整原句，并保留角色详情、混合角色、UNKNOWN、推断及重叠标记；对齐鸿蒙展示，不将可读性改善宣称为身份精度修复。
- ASR final/last/complete 与角色窗口冻结契约不变。通用角色身份精度及 Android 后台/锁屏可靠性仍未通过验收。迁移见 `UPGRADE_0.3.7.md`。

## [0.3.6] - 2026-09-17

- 相对 0.3.5，更新主讲人相对音量准入和有真实取样支持的最终身份匹配，修复已验证录音中的主要发言串人；默认角色上限仍为 4。
- 保留完整词语和真实换人；同句有限 UNKNOWN 补全显式返回 `speakerInferred=true`、`confidence=0`，原始声学时间线不变。
- 生命周期和多批角色回调沿用上一版。短插话、轻声和重叠身份精度仍有限制，通用角色精度验收未通过；范围及迁移见 `UPGRADE_0.3.6.md`。

## [0.3.5] - 2026-09-16

- 接续 0.3.4 预览能力；上一正式交付为 0.3.3，完整范围见客户 `UPGRADE_0.3.5.md`。
- token 时间戳和鼎桥 beginTime/endTime 改为 session 输入音频时间轴，跨句不归零；业务端应移除自行叠加的时间补偿。
- 修复角色尾窗等待期间空兜底覆盖真实尾句；保持唯一 last 后唯一 complete。
- 同步角色身份证据、短片段建档与文本分窗修复，以及语音结束事件、初始等待、冷启动回灌和资源回收修复。
- Demo 显示角色中间/最终状态并保留 SDK 编号，支持文件输入，会议 vadEnd 为 800ms。
- 提供 Release 和 Diagnostics SDK、Demo 及独立源码。四人重叠发言精度仍未通过，不以生命周期通过替代精度验收。
...
- 新增脚本 `asr/tools/09_export_bbpe_vocab.py`：把 google SentencePiece protobuf `.model` 转成 ssentencepiece 期望的两列文本 `.vocab`；模型导出阶段一次性运行即可。
- 商用离线装机 License（零网络）：`AmphionRuntime.init` 接入离线验签，新增公开 API `AmphionRuntime.licenseStatus()` 返回 `AmphionLicenseStatus`；`AmphionOptions` 扩展 `license` / `licenseAssetName`（默认 `amphion-license.lic`）/ `expiryGraceDays` / `licenseEnforcement`（`ENFORCE` 默认 / `PERMISSIVE`）。`.lic` 用 ECDSA P-256 + SHA256（minSdk 24 全覆盖）离线验签，正式设备白名单 license 默认校验签名、到期日、SDK 大版本 / 维护期、功能模块和设备 SN 白名单；applicationId / bundleName 仅作签发记录，不作为授权限制；签名证书 SHA-256 仅在 license 显式写入时校验。签名覆盖 payload 原始字节以规避 canonical JSON 歧义。公钥经 `gradle.properties.AMPHION_LICENSE_PUBLIC_KEY` 构建期注入 `BuildConfig`，为空时 SDK 处于 `DEV_UNLICENSED` 不校验（开发 / 内部构建无感）。
- 新增授权错误码 6001-6010（`LICENSE_MISSING` / `LICENSE_MALFORMED` / `LICENSE_SIGNATURE_INVALID` / `LICENSE_APP_MISMATCH` 保留 / `LICENSE_CERT_MISMATCH` / `LICENSE_EXPIRED` / `LICENSE_DEVICE_MISMATCH` / `LICENSE_SDK_MAJOR_MISMATCH` / `LICENSE_MAINTENANCE_EXPIRED` / `LICENSE_FEATURE_MISSING`），同步登记到 `shared/api-spec/errcodes.yaml` 6xxx 授权段（避开 4xxx 网络段）。
- 新增离线签发工具 `asr/tools/license/`：`gen_keypair.py` / `issue_license.py` / `verify_license.py` / `selftest.sh` + README；私钥与签发产物已加入 `.gitignore`。Python(cryptography) 签发与 JVM(java.security) 验签已实测互通。
...
- 新增脚本 `asr/tools/08_pack_sdk_assets.sh`：把 5 类资产打进 SDK assets，缺失项自动调用 00_push_punct_model.sh / 00_push_weitn_fsts.sh 拉取。详见 `docs/DELIVERY.md`。
...
- Sample app 新增「中英 / 粤英」RadioGroup，按 manifest.lang 自动绑定对应模型；切换时自动 stop 当前监听 + close 旧 engine + 用对应 modelDir 重建。详见 [README.md](../README.md) 的快速开始与 [asr/tools/MODEL_LAYOUT.md](../../../asr/tools/MODEL_LAYOUT.md) 的「同时部署多语言模型」一节。
...
- 新增 iOS 公开 API `WeitnConfig` / `WeitnEngine`（与 Android 对齐）；底层通过 Swift wrapper `SherpaOnnxWetextItnWrapper` 调 C-API；xcframework 由 `asr/ios/build_xcframework.sh` 一键产出后自动带 wetext。
- Sample app 新增 `WeitnAssetInstaller` 与 [asr/tools/00_push_weitn_fsts.sh](../../../asr/tools/00_push_weitn_fsts.sh)：脚本支持两种 fst 来源（本机 pip 编译 / 环境变量直拉预编译产物），push 到 external `asr-weitn-import/`，sample 启动时一次性搬到 internal `<filesDir>/asr-weitn/{zh_itn_tagger.fst,zh_itn_verbalizer.fst}`。MainActivity 把 sw_itn 接管为 WeText 开关：fst + engine 双就绪后自动开一次；ASR final 出来后按 `WeitnEngine.normalize → PunctuationEngine.addPunctuation` 顺序异步刷 UI。
...
- Sample app 新增「标点 (CT-Transformer)」Switch：默认关，开启时异步加载 ~62 MB INT8 模型（~1 秒），关闭时立即释放 ~70 MB native 内存。final 文本先以无标点版立刻显示，标点 ~30 ms 后回来按行 id 替换；标点失败时静默回退。录音 / 切换中 Switch 灰禁。模型由 `asr/tools/00_push_punct_model.sh` 下载（cache + sha256 校验）+ adb push 到 external，sample 启动时 `PunctModelInstaller` 一次性搬到 internal `<filesDir>/asr-punct/`。详见 [README.md](../README.md) 的「可选：开启标点（CT-Transformer 中英双语）」与 [asr/tools/MODEL_LAYOUT.md §7](../../../asr/tools/MODEL_LAYOUT.md)。
...
- 同步删除 sample 端 `ItnPostprocess` / `ItnPostprocessTest` / `ItnAssetInstaller` / sample assets `asr-itn/` / `asr/tools/00_download_itn_fst.sh`：WeText ITN 内置了「幺/两 → 1/2」cardinal 规则，sample 侧的"幺补丁"逻辑不再需要。
