...
- 角色定稿按完整原 ASR 句子输出；句内多人、重叠或未解决的未知不强归单一人物。`secondarySpeakerIndexes` 可记录句内人物，不能单凭非空判定重叠，迁移见 `UPGRADE_0.3.8.md`。
- 改善已确认客户录音中的同人重复建档、早期声音漏建档及部分主讲漏标；增加端侧 CAMPPlus 互补声纹证据，恢复客户离线录音中的角色4。MOSS仅用于研发标注，不进入SDK。
- 保留原始角色时间线、有限 UNKNOWN 推断标记和已提交窗口冻结。ASR final/last/complete 生命周期不变；修复范围为客户录音收益，剩余身份误认、未知及性能代价在升级说明中保留。
