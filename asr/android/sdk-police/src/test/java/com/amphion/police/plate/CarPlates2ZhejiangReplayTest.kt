...
/** 浙江 car_plates2 回放失败归因。 */
class CarPlates2ZhejiangReplayTest {
    ...
    @Test
    fun replay_zhejiang_analyze_remaining() {
        val norm = normalizer()
        ...
        for ((case, row) in cases.zip(evalRows)) {
            if (case.third != "zhejiang") continue
        ...
        sb.appendLine("=== Zhejiang replay analysis ===")
        sb.appendLine("baseline_replay=140/180 (77.8%)")
        ...
        sb.appendLine("delta_replay_vs_baseline=${replayOk - 140}")
        ...
        evalDir.resolve("metrics_zhejiang_replay_analysis.txt").writeText(sb.toString())
        println(sb.toString())
    }

    private fun classifyFailure(exp: String, got: String, asr: String): String {
        if (got.isNotEmpty() && got.take(2) != exp.take(2)) return "wrong_province"
        if (got.isNotEmpty() && got.length >= 2 && exp.length >= 2 && got[1] != exp[1]) {
            return "letter_swap"
        }
        if (got.isEmpty()) {
            when {
                Regex("G\\s*[A-Z]|G\\s+F").containsMatchIn(asr) -> return "no_extract_g_prefix"
                Regex("这|浙记|浙币|浙耳|浙江|这第").containsMatchIn(asr) &&
                    !Regex("浙[A-Z]").containsMatchIn(asr) ->
                    return "no_extract_zhe_homophone"
                Regex("160\\s*538|163\\s*745").containsMatchIn(asr) -> return "no_extract_digit_gap"
