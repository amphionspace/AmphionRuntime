...
/** 河南 car_plates2 回放失败归因。 */
class CarPlates2HenanReplayTest {
    ...
    @Test
    fun replay_henan_analyze_remaining() {
        val norm = normalizer()
        ...
        sb.appendLine("=== Henan replay analysis ===")
        sb.appendLine("baseline_replay=122/285 (42.8%)")
        ...
        sb.appendLine("delta_replay_vs_baseline=${replayOk - 122}")
        ...
        evalDir.resolve("metrics_henan_replay_analysis.txt").writeText(sb.toString())
        println(sb.toString())
    }

    private fun classifyFailure(exp: String, got: String, asr: String): String {
        if (got.isNotEmpty() && got.take(2) != exp.take(2)) {
            return if (got.startsWith("粤")) "wrong_province_yue" else "wrong_province_other"
        }
        ...
        if (got.isEmpty()) {
            when {
                Regex("(?i)U\\s*[A-Z]|U\\s+[A-Z]|UC |UK |UM |UQ |US |UH |UL |UF ").containsMatchIn(asr) ->
                    return "no_extract_u_prefix"
                Regex("育|遇|与|预|玉|裕|寓|喻|御|欲|阅|月|粤").containsMatchIn(asr) &&
                    !Regex("豫[A-Z]").containsMatchIn(asr) ->
                    return "no_extract_yu_homophone"
                Regex("279\\s*406|799\\s*406|292\\s*631").containsMatchIn(asr) ->
