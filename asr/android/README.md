...
零基础完整端到端走一遍，请看 `asr/tools/QUICKSTART.md`。下面假设你已经装好工具链：
...
│   └── src/main/
│       ├── AndroidManifest.xml
│       ├── jniLibs/<abi>/      # 由 asr/tools/05_package_aar_libs.sh 填充
│       ├── assets/amphion-models/    # 由 asr/tools/08_pack_sdk_assets.sh 填充
│       │   ├── zh-en/v1/             # 中英 ASR
│       │   ├── yue-en/v1/            # 粤英 ASR
...
1. native 层（运行时）：`libsherpa-onnx-jni.so` + `libonnxruntime.so`，由
   `asr/tools/04_build_android_so.sh` 在 `sherpa-onnx` 仓库根目录执行 NDK 交叉编译产生。
