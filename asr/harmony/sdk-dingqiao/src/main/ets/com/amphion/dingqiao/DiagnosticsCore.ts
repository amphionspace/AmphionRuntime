...
class DiagnosticSession {
  ...
  hasNonEmptyFinal: boolean = false;
...
export class DiagnosticsCore {
  ...
  record(sourceSessionId: string, engineId: string, event: string,
    ...
    if (event === 'FINISH_REQUESTED' || event === 'AUTO_FINISH_REQUESTED') {
      session.finishIntent = true;
    }
    if (event === 'CALLBACK_RESULT' && fields['isFinal'] === true &&
      Number(fields['textChars'] ?? 0) > 0) {
      session.hasNonEmptyFinal = true;
    }
