...
struct LacHolder {
  std::unique_ptr<LacPersonNer> value;
};

void FinalizeLac(napi_env, void* data, void*) { delete static_cast<LacHolder*>(data); }

LacHolder* GetHolder(napi_env env, napi_value value) {
  void* data = nullptr;
  if (napi_get_value_external(env, value, &data) != napi_ok || data == nullptr) {
    throw std::runtime_error("invalid LAC person NER handle");
  }
  return static_cast<LacHolder*>(data);
}

napi_value CreateLac(napi_env env, napi_callback_info info) {
  try {
    size_t argc = 5;
    ...
    if (argc != 5) {
      throw std::runtime_error(
          "createLacPersonNer expects model, transitions, word, tag, and q2b paths");
    }
    auto holder = std::make_unique<LacHolder>();
    holder->value = std::make_unique<LacPersonNer>(
        GetString(env, args[0], "modelPath"), GetString(env, args[1], "transitionsPath"),
        GetString(env, args[2], "wordPath"), GetString(env, args[3], "tagPath"),
        GetString(env, args[4], "q2bPath"));
    napi_value external = nullptr;
    napi_create_external(env, holder.get(), FinalizeLac, nullptr, &external);
    holder.release();
    return external;
...
napi_value FindPersons(napi_env env, napi_callback_info info) {
  try {
    size_t argc = 2;
    napi_value args[2] = {nullptr, nullptr};
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    if (argc != 2) throw std::runtime_error("findLacPersonSpans expects handle and text");
    const std::vector<PersonSpan> spans = GetHolder(env, args[0])->value->FindPersonSpans(
        GetString(env, args[1], "text"));
    napi_value output = nullptr;
    napi_create_array_with_length(env, spans.size(), &output);
    for (size_t i = 0; i < spans.size(); ++i) {
      napi_value item = nullptr;
      napi_create_object(env, &item);
      napi_value start = nullptr;
      napi_value end = nullptr;
      napi_create_int32(env, spans[i].start, &start);
      napi_create_int32(env, spans[i].end, &end);
      napi_set_named_property(env, item, "start", start);
      napi_set_named_property(env, item, "end", end);
      napi_set_element(env, output, i, item);
    }
    return output;
  } catch (const std::exception& error) {
    ThrowJsError(env, error.what());
...
napi_value CloseLac(napi_env env, napi_callback_info info) {
  try {
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    if (argc != 1) throw std::runtime_error("closeLacPersonNer expects a handle");
    GetHolder(env, args[0])->value.reset();
...
void RegisterLacPersonNer(napi_env env, napi_value exports) {
  napi_property_descriptor descriptors[] = {
      {"createLacPersonNer", nullptr, CreateLac, nullptr, nullptr, nullptr, napi_default, nullptr},
      {"findLacPersonSpans", nullptr, FindPersons, nullptr, nullptr, nullptr, napi_default, nullptr},
      {"closeLacPersonNer", nullptr, CloseLac, nullptr, nullptr, nullptr, napi_default, nullptr},
  };
