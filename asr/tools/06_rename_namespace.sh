...
#
# 替换范围：
#   - asr/android/gradle.properties    AMPHION_RUNTIME_GROUP_ID 行
#   - asr/android/{sdk,sample}/**.kt   import / package
...
#   - asr/tools/*.sh / *.md 中 SAMPLE_PKG=com.amphion.asr.sample（需要先决定 sample applicationId 是否还叫 sample）
#
# 用法：
#   bash asr/tools/06_rename_namespace.sh --group-id com.example
#   bash asr/tools/06_rename_namespace.sh --group-id com.example --pkg-prefix com.example.asr
#   bash asr/tools/06_rename_namespace.sh --group-id com.example --dry-run
#
# 选项：
...
2. 同步检查并人工修改下列地方（脚本不动它们）：
   ...
   - $ROOT/asr/tools/*.sh：SAMPLE_PKG / model_id 等业务标识是否需要同步换
