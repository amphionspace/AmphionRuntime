...
本文是按 实际真机验证过的成功路径 写的，每一步都给可以直接 copy 的命令，并把每个验证点和失败时的处置写在旁边。如果你只想集成 SDK 不想从源码构建，请看 `asr/android/docs/INTEGRATION.md`。
...
```bash
ls -lh asr/tools/demo-model/zipformer_L_zh_en/
# 期望看到：
...
# 如果连了多个设备，明确指定：
# bash asr/tools/00_fetch_demo_model.sh push 192.168.1.100:42123
```
...
| 症状 | 根因 | 处置 |
| --- | --- | --- |
| `bash asr/tools/04_build_android_so.sh: line 31: LATEST_TAG。: unbound variable` | bash 在中文标点旁不能正确截断变量名 | 已修复（${VAR} 显式界定）。如自己写脚本注意：变量后接中文标点必须 `${VAR}` 不能 `$VAR` |
...
调用方在代码里显式调用 `.decodingMethod(...)` / `.maxActivePaths(...)` 的优先级最高，会盖掉 manifest。详见 `asr/android/docs/INTEGRATION.md` 第 7 节。
...
# 再 push 自己的
bash asr/tools/00_push_my_model.sh --src ~/my-asr-models/.../1.0.1 \
