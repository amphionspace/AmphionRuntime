...
| `sync_automatic_agc_evidence.py` | A | 只读校验 AGC 完整评测与 native 构建依赖、跨端 ingress/processor/backend、bridge 和 evaluator 的 SHA-256 绑定；session 的 VAD、声纹和生命周期接入由轻量信号域契约覆盖 |
