...
# asr/tools/08_pack_sdk_assets.sh 的输入准备步骤（自动校验 sha256 + 解压到 cache）。
# adb push 路径在 0.2.0 之后不再被 sample app 读取，仅用于 0.1.x 兼容性自验。
#
# 设计：
# - 把 tarball 缓存到 asr/tools/punct-model/<file>.tar.bz2（已被根 .gitignore 排除）
...
#
# 用法：
#   bash asr/tools/00_push_punct_model.sh
#   bash asr/tools/00_push_punct_model.sh --serial <adb-serial>
#   bash asr/tools/00_push_punct_model.sh --pkg com.example.fork.asr  # fork 改名后
#   bash asr/tools/00_push_punct_model.sh --no-push                   # 只下不 push
#
# 可选环境变量：
...
CACHE_DIR="${REPO_ROOT}/asr/tools/punct-model"
