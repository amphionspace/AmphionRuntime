...
class HarmonyRuntimeIdentityTest(unittest.TestCase):
    ...
    def test_runtime_identity_matches_delivery_version(self) -> None:
        self.run_identity(
            """
            assert.equal(HARMONY_SDK_VERSION, '0.3.13');
            assert.equal(HARMONY_SDK_MAJOR, 1);
            assert.equal(HARMONY_SDK_RELEASE_DATE, '2026-09-08');
            """
        )
    ...
    def test_delivery_ledger_keeps_last_actual_delivery_until_packaging(self) -> None:
        ...
        self.assertEqual(harmony_deliveries[-1]["version"], "0.3.13")
    ...
    def test_license_major_and_maintenance_boundaries_fail_closed(self) -> None:
        self.run_identity(
            ...
            assert.equal(
                evaluateLicenseIdentity(1, HARMONY_SDK_MAJOR, '2026-08-26', HARMONY_SDK_RELEASE_DATE),
              LicenseIdentityFailure.MAINTENANCE_EXPIRED,
            );
            assert.equal(
    ...
    def test_native_type_package_and_locks_match_runtime_version(self) -> None:
        for relative in (
            ...
            self.assertEqual(manifest["version"], "0.3.13", relative)
        ...
        for relative in (
            ...
            self.assertTrue(
                all(package["version"] == "0.3.13" for package in native_packages),
                relative,
            )
