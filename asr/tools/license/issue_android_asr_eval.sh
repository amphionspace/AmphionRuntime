#!/usr/bin/env bash
# Issue the generic Android ASR evaluation license: four calendar months,
...
OUT="${1:-$ROOT/../delivery/amphion-android-asr-eval.lic}"
...
"$PYTHON" "$ROOT/tools/license/issue_license.py" \
  ...
  --customer "Amphion Android ASR Evaluation" \
  --license-id "ANDROID-ASR-EVAL-$(date +%Y%m%d)" \
  --issued "$ISSUED" \
  --expires "$EXPIRES" \
  --device-id-salt-id "" \
  --features ASR \
...
echo "[ok] generic Android ASR license: issued=$ISSUED expires=$EXPIRES app=unbound device=unbound"
