...
# asr/tools/08_pack_sdk_assets.sh 的输入准备步骤（pip 编译或 CDN 拉取到 cache）。
...
#   bash asr/tools/00_push_weitn_fsts.sh --serial <adb-serial>
#   bash asr/tools/00_push_weitn_fsts.sh --pkg com.example.fork.asr  # fork 改名后
#   bash asr/tools/00_push_weitn_fsts.sh --no-push                   # 只编不 push
