#!/usr/bin/env bash
# 下载 TS-ASR 调研期所需的 3 个模型（不含 ASR；ASR 复用 asr/tools/ 已导出的流式 zipformer）：
#
...
#   bash asr/tools/speaker/00_download_models.sh --only vad
#   bash asr/tools/speaker/00_download_models.sh --mirror https://your-mirror.example/sherpa-onnx
#
# 环境变量：
#   SPEAKER_MIRROR  替代默认的 github 下载源；URL 末尾不带 /
#   SPEAKER_DEST    覆盖默认下载目录（默认 asr/tools/speaker/models/）
...
下一步：
  ...
  2. bash asr/tools/speaker/01_enroll_target.py --help

注意：本目录 ($DEST) 已被 .gitignore 排除，不会进 git 历史。
      模型分发与 SDK 打包请走 asr/tools/08_pack_sdk_assets.sh 风格的独立流程，本调研不参与。
EOF
