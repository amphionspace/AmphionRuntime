...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      [[ -z "$VERSION_ARG" ]] || { echo "[ERROR] multiple versions provided" >&2; exit 2; }
      VERSION_ARG="$1"
      shift
      ;;
  esac
...
if [[ "$SDK_ONLY" != true && "${DINGQIAO_DEMO_SIGN_DEBUG:-}" == "1" ]]; then
...
elif [[ "$SDK_ONLY" != true && -n "${DINGQIAO_DEMO_KEYSTORE:-}" ]]; then
...
if [[ "$SDK_ONLY" == true ]]; then
  dingqiao_write_version_txt "$OUT_ROOT/VERSION.txt" \
    ...
    "pack_script=asr/tools/delivery/pack_dingqiao_customer_delivery.sh --sdk-only"
else
...
不包含粤英模型，不包含 Demo APK 或 Demo 源码，不包含 TTS SDK、TTS 模型或授权文件。
公共商用授权文件 amphion-license.lic 继续沿用上次交付版本。

目录
----
  aar/$AAR_NAME    集成用 SDK（~${AAR_MB} MB）
  docs/             集成、接口、授权说明、更新日志和第三方声明
  VERSION.txt       版本信息
  CHECKSUMS.txt     全部交付文件 SHA-256

快速集成
--------
  1. 将 aar/$AAR_NAME 放入正式 App 的 libs/ 目录
  2. implementation(files("libs/$AAR_NAME"))
  3. 按 docs/DINGQIAO_INTEGRATION.md 初始化 SpeechRecognizeSdk
  4. 将既有 amphion-license.lic 放入正式 App 的 assets/ 目录
EOF
else
...
(root / "CHECKSUMS.txt").write_text("".join(lines), encoding="utf-8")
PY
fi
