...
// 2) 业务方下载 SherpaAsrSdk.xcframework + 把 SDK Swift 源码作为子目录嵌入
// 3) 通过 CocoaPods 集成（见 SherpaAsrSdk.podspec）
//
...
let package = Package(
    ...
    products: [
        .library(name: "SherpaAsrSdk", targets: ["SherpaAsrSdk"]),
    ],
    dependencies: [
    ...
    targets: [
        ...
        .binaryTarget(
            ...
            // 替换为你公司私有 OSS 的真实地址：
            // url: "https://your-cdn.example.com/sherpa-asr-sdk/0.1.0/SherpaAsrSdk.xcframework.zip",
            // checksum: "REPLACE_ME_WITH_REAL_SHA256",
            // 临时本地引用（CI 跑 build_xcframework.sh 后会产出在这个相对路径）：
            path: "SherpaAsrSdk.xcframework"
        ),
    ]
)
