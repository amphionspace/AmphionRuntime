...
3. 用 Xcode 打开 `ios/SherpaAsrSdk/Package.swift`（SPM 包）
4. 创建一个新的 SwiftUI App target，复制 `Sample/SherpaAsrSdkSampleApp.swift` 与 `ContentView.swift` 进去
5. 给 Sample target 添加：
   - `NSMicrophoneUsageDescription` -> "本 demo 需要使用麦克风演示语音识别"
   - 依赖 SPM Library: `SherpaAsrSdk`
6. 把模型目录拷到设备 / 模拟器：
