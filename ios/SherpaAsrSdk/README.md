## SherpaAsrSdk for iOS

公司 ASR SDK 的 iOS 实现，与 [Android SherpaAsrSdk](../../android/SherpaAsrSdk/) 公开 API 一一对应。
...
├── README.md
├── SherpaAsrSdk.podspec           # CocoaPods 兼容
├── build_xcframework.sh           # 调用上游 build-ios.sh 产出 SherpaAsrSdk.xcframework
├── Sources/
│   ├── SherpaAsrSdk/              # 主模块（公开 API + 内部实现）
│   │   ├── AsrSdk.swift
│   │   ├── AsrConfig.swift
│   │   ├── AsrEngine.swift
│   │   ├── AsrSession.swift
...
`https://your.git.host/your-org/sherpa-asr-sdk-ios.git`

或在 Package.swift 中：

```swift
.package(url: "https://your.git.host/your-org/sherpa-asr-sdk-ios.git", from: "0.1.0"),
```

### CocoaPods（兼容）

```ruby
pod 'SherpaAsrSdk', '~> 0.1'
```
