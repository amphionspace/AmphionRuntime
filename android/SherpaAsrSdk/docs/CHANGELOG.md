...
仓库拆分首版（在新 sherpa-asr-sdk 独立仓库内的第一个 tag）。
...
- 仓库迁移：从 sherpa-onnx 主仓内的子目录迁移为独立仓库 sherpa-asr-sdk；sherpa-onnx 通过 third_party/sherpa-onnx submodule 引用，公司侧不再 patch sherpa-onnx 源码。
