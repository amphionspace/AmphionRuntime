...
适用 SDK：`com.yourco:asr-sdk` 0.1.0
...
| 项 | 取值 |
| --- | --- |
| SDK 名称 | YourCo ASR SDK |
| 包名 | `com.yourco:asr-sdk` |
| 版本 | 0.1.0 |
| 提供方 | YourCo（请替换为你公司的完整名称与联系方式） |
| 隐私政策链接 | https://your-domain.example.com/privacy/asr-sdk |
...
> 我们的 App 集成了 YourCo ASR SDK 用于 离线 语音识别。SDK 仅在用户主动开启录音时处理音频数据，识别完成即丢弃，不上传至任何服务器，不收集设备唯一标识符、位置等其他个人信息。当你需要更新本地识别模型时，App 会向 我方服务器（仅由 我方域名 提供） 下载模型文件；除此之外 SDK 不与任何第三方进行网络通信。
