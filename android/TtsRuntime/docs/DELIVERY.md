...
- `android/TtsRuntime/`
- `tools/tts/verify_lits_delivery_16k_package.py`
- `tools/tts/README.md`
- `tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0/.gitkeep`

源码仓库中不保留：

- `tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0/` 里的真实 `.onnx/.json/.txt/.wav`
- `local.properties`
- `.gradle/`
- `build/`
...
- 如果需要补充专有名词固定读音，统一按 [../../../tools/tts/README.md](../../../tools/tts/README.md) 里的“专有名词怎么改”处理
...
- 文件名是否完整
- 目录层级是否正确
- 是否把模型放到了 `tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0/`
...
- 在 `preBuild` 阶段把 `tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0/` 中的模型文件同步到 `sdk/src/main/assets/lits-models/tts/lits_delivery_16k_hifigan/1.0.0/`
...
```bash
bash tools/tts/android/pack_lits_tts_android_delivery.sh 0.1.0
```
...
```text
tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0/
```
...
- 私钥 / `.pem`
- 生成的 `android/TtsRuntime/sdk/src/main/assets/`

可用下面命令校验交付目录或 zip：

```bash
bash tools/tts/android/verify_lits_tts_android_delivery.sh ../delivery/lits-tts-android-sdk-v0.1.0/
bash tools/tts/android/verify_lits_tts_android_delivery.sh ../delivery/lits-tts-android-sdk-v0.1.0-YYYYMMDD.zip
```
...
- 构建前模型包是否已放到 `tools/tts/trial-export/...`
