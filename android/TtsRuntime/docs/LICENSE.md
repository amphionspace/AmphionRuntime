# Lits TTS 离线 License 机制

面向 To B 私有化交付的纯离线（零网络）授权。本文讲机制与设计取舍；签发 / 校验的操作步骤见
`tools/tts/license/README.md`，集成步骤见 `INTEGRATION.md` 第 11 节。

## 1. 目标与边界

- 离线：终端零联网即可校验；我方不运营任何在线授权服务。
- 防转移：license 与宿主 applicationId 绑定（必），可选叠加签名证书、单机设备指纹、到期。
...
| 字段 | 含义 | 空值语义 |
|---|---|---|
| applicationId | 绑定宿主包名（必填） | 不可空 |
| certSha256 | 绑定签名证书 SHA-256 | 空=不绑证书 |
| deviceSha256 | 绑定设备指纹 | 空=不绑单机 |
| expiresAt | 到期日 yyyy-MM-dd | 空=永久买断 |
| customer / licenseId / installTier / features / issuedAt / sdkMajor | 声明性信息 | 仅展示 / 审计 |

设备指纹算法：SHA-256("{applicationId}|{ANDROID_ID}")，大写 hex、无冒号。

## 3. SDK 端校验流程

入口 `LicenseVerifier.verify(ctx, ...)` 解析宿主 packageName / 签名证书 / 设备指纹后委托
...
8. deviceSha256 非空且不符 → LICENSE_DEVICE_MISMATCH（1002300018）
9. 全部通过 → LICENSED
...
gradle 属性 `LITS_TTS_LICENSE_PUBLIC_KEY` → `BuildConfig.LICENSE_PUBLIC_KEY_B64`：

- 留空（默认）：开发 / 内部构建，不武装。
- 正式交付：填入 gen_keypair.py 生成的公钥；私钥严禁进库。
- 可用 `-PLITS_TTS_LICENSE_PUBLIC_KEY=...` 或环境变量注入，避免公钥进 VCS。
