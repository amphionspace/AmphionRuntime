...
internal object LitsTtsFrontend {
    ...
    private fun resources(layout: LitsTtsAssetInstaller.InstalledLayout): FrontendResources =
        resourcesByRoot.getOrPut(layout.rootDir.absolutePath) {
            FrontendResources(
                wordPinyin = loadWordPinyin(layout),
                polyphonicWords = layout.polychar.readLines(Charsets.UTF_8)
                    .map { it.trim() }
                    .filter { it.isNotEmpty() }
                    .toSet(),
                maxWordLength = loadWordPinyin(layout).keys.maxOfOrNull { it.length } ?: 1,
                cmudict = loadCmudict(layout),
                symbolToId = loadSymbols(layout),
                pinyinToTokens = loadTokenMap(layout.pinyinToTokens, "pinyin_to_tokens"),
                arpabetToTokens = loadTokenMap(layout.arpabetToTokens, "arpabet_to_tokens"),
            )
        }
