...
> **2026-08-07 复测结论：** 扩展真机语料后确认，该能力的 SDK 生命周期稳定，固定 C1/C2/C3
> 重叠用例也能稳定保留关键内容；但目标人缺席的多人重叠或交通噪声中出现了 3/12 条错误正式文本，
> 且部分目标人在场音频存在丢字、截断。当前必须保持显式可选，不能作为普通 ASR 的全局默认路径，
> 也不能作为“只允许注册人说话”的身份安全边界。完整复测数据、复现和问题分析见
> [《Speaker VAD Enhance 真实表现（2026-08-07）》](./SPEAKER_VAD_ENHANCE_DEVICE_EVAL_20260807.md)。
