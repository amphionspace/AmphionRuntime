...
对明确属于“目标人说完、其他人才接话”的场景，使用现有 `1000/300 ms` Speaker VAD 参数作为显式
策略，而不是改全局默认值。它复用当前 ERes2Net，不新增模型、runtime 或内存边界；当前 Mate 80 已完成
真机资源和 C1 业务验证。

上线前仍要补一组独立 target→other 语料，确认缩短窗口没有截断目标人尾字。C2/C3 是同时重叠，不应
被宣传为这一策略可以解决。
