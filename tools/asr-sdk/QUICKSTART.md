...
本文是按 实际真机验证过的成功路径 写的，每一步都给可以直接 copy 的命令，并把每个验证点和失败时的处置写在旁边。如果你只想集成 SDK 不想从源码构建，请看 `android/SherpaAsrSdk/docs/INTEGRATION.md`。
...
强烈推荐先用 demo 模型把工程跑通，再换自己的 ONNX。理由见 `tools/asr-sdk/README.md`。

```bash
cd /Users/boxp/workspace/sherpa-asr-sdk
bash tools/asr-sdk/00_fetch_demo_model.sh
```
...
```bash
ls -lh tools/asr-sdk/demo-model/sherpa-onnx-streaming-zh-en-demo/1.0.0/
# 期望看到：
...
```bash
bash tools/asr-sdk/05_package_aar_libs.sh
```

会把上一步的两个 .so 拷到 `android/SherpaAsrSdk/sdk/src/main/jniLibs/arm64-v8a/`。armeabi-v7a 没编，会被 SKIP，是预期行为。
...
```bash
cd /Users/boxp/workspace/sherpa-asr-sdk/android/SherpaAsrSdk
...
# 如果连了多个设备，明确指定：
# bash tools/asr-sdk/00_fetch_demo_model.sh push 192.168.1.100:42123
```

会把 4 个模型文件 + manifest.json 推到设备的：

```
/sdcard/Android/data/com.yourco.asr.sample/files/asr-models-import/
...
AsrSdk: OnlineRecognizer loaded from /data/user/0/com.yourco.asr.sample/files/asr-models/sherpa-onnx-streaming-zh-en-demo/1.0.0
```
...
| 症状 | 根因 | 处置 |
| --- | --- | --- |
| `bash tools/asr-sdk/04_build_android_so.sh: line 31: LATEST_TAG。: unbound variable` | bash 在中文标点旁不能正确截断变量名 | 已修复（${VAR} 显式界定）。如自己写脚本注意：变量后接中文标点必须 `${VAR}` 不能 `$VAR` |
...
# 编 + 拷
bash tools/asr-sdk/04_build_android_so.sh arm64-v8a    # 增量编译，2–5 分钟
bash tools/asr-sdk/05_package_aar_libs.sh
...
- 文件名是 EngineImpl 写死的（`sdk/src/main/java/com/yourco/asr/internal/EngineImpl.kt:131-136`）。如果你的 icefall 输出叫 `encoder-epoch-99-avg-1.int8.onnx`，必须在本地先 mv 重命名好。
...
调用方在代码里显式调用 `.decodingMethod(...)` / `.maxActivePaths(...)` 的优先级最高，会盖掉 manifest。详见 `android/SherpaAsrSdk/docs/INTEGRATION.md` 第 7 节。
...
```bash
cd /Users/boxp/workspace/sherpa-asr-sdk
bash tools/asr-sdk/00_push_my_model.sh \
...
3. 把整个目录 push 到 `/sdcard/Android/data/com.yourco.asr.sample/files/asr-models-import/<id>/<v>/`
4. 提示重启命令
...
```121:128:android/SherpaAsrSdk/sample/src/main/java/com/yourco/asr/sample/MainActivity.kt
...
```bash
# 一次性清掉所有已导入模型
adb shell run-as com.yourco.asr.sample rm -rf files/asr-models
adb shell rm -rf /sdcard/Android/data/com.yourco.asr.sample/files/asr-models-import

# 再 push 自己的
bash tools/asr-sdk/00_push_my_model.sh --src ~/my-asr-models/.../1.0.1 \
...
AsrSdk: OnlineRecognizer loaded from /data/user/0/com.yourco.asr.sample/files/asr-models/asr-streaming-zipformer-zh-en/1.0.1
```
