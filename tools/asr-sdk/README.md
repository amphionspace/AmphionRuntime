...
本目录是为「基于 sherpa-onnx 的 Android ASR SDK」准备的 工程辅助脚本与文档，覆盖阶段 A（模型导出与验证）和阶段 B（Android 原生层 .so 编译与打包）。Android 工程在 `android/SherpaAsrSdk/`。
...
```bash
open tools/asr-sdk/QUICKSTART.md   # 推荐入口
```
...
bash tools/asr-sdk/03_verify_onnx.sh \
...
# 可选：bash tools/asr-sdk/04_build_android_so.sh armeabi-v7a

# B.3 拷贝到 SDK 工程的 jniLibs/
bash tools/asr-sdk/05_package_aar_libs.sh

# ----- 阶段 C / D：见 android/SherpaAsrSdk/ -----
# C：SDK 模块（com.yourco.asr）+ 公开 API + 内部实现 + 模型下载
