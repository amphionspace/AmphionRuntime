...
#
# 用法：
#   bash tools/asr-sdk/05_package_aar_libs.sh                  # 拷已经存在的全部 ABI
#   bash tools/asr-sdk/05_package_aar_libs.sh arm64-v8a
#   bash tools/asr-sdk/05_package_aar_libs.sh all
#
# 这一步执行完，SDK 工程就具备了完整的 native 依赖，可以直接：
#   cd android/SherpaAsrSdk && ./gradlew :sdk:assembleRelease
...
SDK_JNI_LIBS_DIR="$REPO_ROOT/android/SherpaAsrSdk/sdk/src/main/jniLibs"
...
echo "[NEXT] 进 android/SherpaAsrSdk 执行："
