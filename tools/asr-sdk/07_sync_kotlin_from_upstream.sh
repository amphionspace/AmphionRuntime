...
# 同步到 android/SherpaAsrSdk/sdk/src/main/java/com/k2fsa/sherpa/onnx/。
#
# 何时使用：
...
#
# 用法：
#   bash tools/asr-sdk/07_sync_kotlin_from_upstream.sh           # 实际同步
#   bash tools/asr-sdk/07_sync_kotlin_from_upstream.sh --check   # 只 diff 不写入，CI 用
...
DST_DIR="$REPO_ROOT/android/SherpaAsrSdk/sdk/src/main/java/com/k2fsa/sherpa/onnx"
...
if [[ "$MODE" == "check" ]]; then
  if [[ $DIFF_COUNT -gt 0 ]]; then
    ...
    echo "       Run: bash tools/asr-sdk/07_sync_kotlin_from_upstream.sh"
    exit 2
  fi
