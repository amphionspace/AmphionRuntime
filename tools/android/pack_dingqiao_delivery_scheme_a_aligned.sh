...
#   bash tools/android/pack_dingqiao_delivery_scheme_a_aligned.sh [交付版本号]
...
dingqiao_write_version_txt "$OUT_ROOT/VERSION.txt" \
  ...
  "pack_script=tools/android/pack_dingqiao_delivery_scheme_a_aligned.sh"

bash "$REPO_ROOT/tools/android/verify_dingqiao_delivery.sh" "$OUT_ROOT/VERSION.txt"
bash "$REPO_ROOT/tools/android/verify_dingqiao_delivery.sh" "$OUT_ROOT/aar/$AAR_NAME"
...
正式对鼎桥发包: bash tools/android/pack_dingqiao_customer_delivery.sh
校验溯源: bash tools/android/verify_dingqiao_delivery.sh VERSION.txt
EOF
