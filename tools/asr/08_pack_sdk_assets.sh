...
#
# 资产清单（详见 android/AmphionRuntime/sdk/src/main/assets/amphion-models/README.md）：
...
#
#   ZH_EN_DIR     默认 tools/asr/demo-model/zipformer_L_zh_en
...
#                 如果只有 bbpe.model（protobuf），先跑 tools/asr/09_export_bbpe_vocab.py 转换
#   YUE_EN_DIR    默认 tools/asr/demo-model/zipformer_L_yue_en
#                 内容同上
#   PUNCT_DIR     默认 tools/asr/punct-model/sherpa-onnx-punct-ct-transformer-zh-en-vocab272727-2024-04-12-int8
#                 必须包含 model.int8.onnx
#                 没有时会自动调用 tools/asr/00_push_punct_model.sh --no-push
#   ITN_DIR       默认 tools/asr/weitn-fsts
#                 必须包含 zh_itn_tagger.fst / zh_itn_verbalizer.fst
#                 没有时会自动调用 tools/asr/00_push_weitn_fsts.sh --no-push
#   VAD_FILE      默认 tools/asr/vad-model/silero_vad.onnx
#                 没有时会从 sherpa-onnx 官方 release 自动下载
#
# 用法：
#   bash tools/asr/08_pack_sdk_assets.sh
#   ZH_EN_DIR=/path/to/zh_en bash tools/asr/08_pack_sdk_assets.sh
#
# 输出：
#   - 把上述文件分别拷贝到
#     android/AmphionRuntime/sdk/src/main/assets/amphion-models/<bundle>/v1/
...
ZH_EN_DIR="${ZH_EN_DIR:-${REPO_ROOT}/tools/asr/demo-model/zipformer_L_zh_en}"
YUE_EN_DIR="${YUE_EN_DIR:-${REPO_ROOT}/tools/asr/demo-model/zipformer_L_yue_en}"
PUNCT_DIR="${PUNCT_DIR:-${REPO_ROOT}/tools/asr/punct-model/sherpa-onnx-punct-ct-transformer-zh-en-vocab272727-2024-04-12-int8}"
ITN_DIR="${ITN_DIR:-${REPO_ROOT}/tools/asr/weitn-fsts}"
VAD_FILE="${VAD_FILE:-${REPO_ROOT}/tools/asr/vad-model/silero_vad.onnx}"

ASSET_ROOT="${REPO_ROOT}/android/AmphionRuntime/sdk/src/main/assets/amphion-models"
...
if [[ ! -f "${PUNCT_DIR}/model.int8.onnx" ]]; then
  warn "没找到 ${PUNCT_DIR}/model.int8.onnx，尝试调用 tools/asr/00_push_punct_model.sh --no-push 拉一份缓存"
  bash "${SCRIPT_DIR}/00_push_punct_model.sh" --no-push
fi
ensure_file "${PUNCT_DIR}/model.int8.onnx" \
  "请检查 PUNCT_DIR 或先跑 bash tools/asr/00_push_punct_model.sh --no-push"

# -------- 2. 准备 ITN --------
if [[ ! -f "${ITN_DIR}/zh_itn_tagger.fst" || ! -f "${ITN_DIR}/zh_itn_verbalizer.fst" ]]; then
  warn "没找到 WeText fsts 在 ${ITN_DIR}，尝试调用 tools/asr/00_push_weitn_fsts.sh --no-push 编译一份"
  bash "${SCRIPT_DIR}/00_push_weitn_fsts.sh" --no-push
fi
ensure_file "${ITN_DIR}/zh_itn_tagger.fst" \
  "请检查 ITN_DIR 或先跑 bash tools/asr/00_push_weitn_fsts.sh --no-push"
...
ensure_dir "${ZH_EN_DIR}" \
  "请用 tools/asr/00_fetch_demo_model.sh 拉 demo 模型，或把自有模型目录设到 ZH_EN_DIR 环境变量"
ensure_dir "${YUE_EN_DIR}" \
  "请用 tools/asr/00_fetch_demo_model.sh 拉 demo 模型，或把自有模型目录设到 YUE_EN_DIR 环境变量"

for f in encoder.int8.onnx decoder.onnx joiner.int8.onnx tokens.txt bbpe.vocab; do
  ensure_file "${ZH_EN_DIR}/${f}" "中英 ASR 缺 ${f}（bbpe.vocab 用 tools/asr/09_export_bbpe_vocab.py 从 bbpe.model 导出）"
...
info "下一步：cd android/AmphionRuntime && ./gradlew :sdk:assembleRelease"
info "       AAR 输出：android/AmphionRuntime/sdk/build/outputs/aar/sdk-release.aar"
