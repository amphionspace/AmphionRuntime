...
本目录是为「基于 sherpa-onnx 的 AmphionRuntime」准备的 工程辅助脚本与文档，覆盖阶段 A（模型导出与验证）和阶段 B（Android 原生层 .so 编译与打包）。Android 工程在 `android/AmphionRuntime/`。
...
```bash
open tools/asr/QUICKSTART.md   # 推荐入口
```
...
bash tools/asr/03_verify_onnx.sh \
...
# 可选：bash tools/asr/04_build_android_so.sh armeabi-v7a

# B.3 拷贝到 SDK 工程的 jniLibs/
bash tools/asr/05_package_aar_libs.sh

# ----- 阶段 C / D：见 android/AmphionRuntime/ -----
