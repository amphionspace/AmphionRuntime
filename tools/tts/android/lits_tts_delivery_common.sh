...
lits_tts_android_root_from_repo() {
  local repo_root="$1"
  printf '%s/android/TtsRuntime\n' "$repo_root"
}

lits_tts_default_model_dir() {
  local repo_root="$1"
  printf '%s/tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0\n' "$repo_root"
}
...
lits_tts_assert_reproducible_build() {
  ...
  if [[ "${GIT_DIRTY:-false}" == "true" ]]; then
    ...
    echo "        Local preview: LITS_TTS_ALLOW_DIRTY=1 bash tools/tts/android/pack_lits_tts_android_delivery.sh" >&2
    exit 1
  fi
}
    ...
    for root_path in dirty_roots:
        ...
        for extra in sorted(root_path.rglob("*")):
            ...
            if "build" in parts or ".gradle" in parts:
                continue
            if rel_posix.startswith("android/TtsRuntime/sdk/src/main/assets/"):
                continue
            if rel_posix.startswith("tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0/") and extra.name != ".gitkeep":
                continue
            if extra.is_dir():
...
PY
  local source_model_dir="$out_src/tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0"
...
Build:
  cd android/TtsRuntime
  python ../../tools/tts/verify_lits_delivery_16k_package.py --model-dir ../../tools/tts/trial-export/lits_delivery_16k_hifigan/1.0.0
  ./gradlew :sdk:testDebugUnitTest
  ./gradlew :sdk:assembleRelease

Do not edit android/TtsRuntime/sdk/src/main/assets directly. Gradle copies
the model package from tools/tts/trial-export into Android assets during preBuild.
EOF
}
