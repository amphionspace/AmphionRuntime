...
#
# Usage (repo root):
#   bash tools/tts/android/pack_lits_tts_android_delivery.sh [version]
#
# Optional env:
...
lits_tts_write_version_txt "$OUT_ROOT/VERSION.txt" \
  ...
  "model_version=1.0.0" \
  "pack_script=tools/tts/android/pack_lits_tts_android_delivery.sh"
...
从源码重建
----------
  1. 进入 android-src/TTS/android/TtsRuntime
