...
本目录是 [docs Target_speaker.md](../../android/AmphionRuntime/docs/Target_speaker.md) 调研结论的执行版，落地"方案 A 工程化加固版"MVP，跑通 server 端 Python pipeline 并量化调研文档"已知未知"小节列的 5 项，输出可进入三端工程化或阶段 2/3 演进的决策证据。
...
# 国内访问慢可挂镜像：
# bash tools/speaker/00_download_models.sh --mirror https://your-mirror/sherpa-onnx
```
...
```bash
python tools/speaker/01_enroll_target.py \
  --speaker-model tools/speaker/models/3dspeaker_speech_eres2net_base_sv_zh-cn_3dspeaker_16k.onnx \
  --enroll-wavs path/to/spk1.wav path/to/spk2.wav path/to/spk3.wav \
  --out tools/speaker/data/target_embedding.npy
```

### 3. 跑离线 TS-ASR pipeline

`--asr-model-dir` 指向 `tools/asr/` 已经导出 + 量化好的流式 zipformer modelDir（4 文件命名见 [tools/asr/MODEL_LAYOUT.md](../asr/MODEL_LAYOUT.md)）：

```bash
python tools/speaker/02_ts_asr_offline.py \
  --asr-model-dir <已经验证过的流式 zipformer modelDir> \
  --speaker-model tools/speaker/models/3dspeaker_speech_eres2net_base_sv_zh-cn_3dspeaker_16k.onnx \
  --silero-vad-model tools/speaker/models/silero_vad.onnx \
  --target-embedding tools/speaker/data/target_embedding.npy \
  --input path/to/test1.wav path/to/test2.wav \
  --out tools/speaker/results/test_run.jsonl \
  --print-stdout
```
...
```bash
python tools/speaker/05_rtf_local.py \
  --speaker-models \
    tools/speaker/models/3dspeaker_speech_eres2net_base_sv_zh-cn_3dspeaker_16k.onnx \
    tools/speaker/models/wespeaker_en_voxceleb_CAM++.onnx \
  --bench-wav path/to/any_clean_speech.wav \
  --window-secs 1.0 2.5 5.0 \
  --warmup 2 --runs 10 \
  --out tools/speaker/results/rtf_local.json
```
...
python tools/speaker/03_eval.py \
  --cuts /path/to/ts_hw_test_cuts_all.jsonl.gz \
  --asr-model-dir tools/asr/demo-model/zipformer_L_zh_en \
  --speaker-model tools/speaker/models/3dspeaker_speech_eres2net_base_sv_zh-cn_3dspeaker_16k.onnx \
  --out tools/speaker/results/eval_sanity.jsonl \
  --max-n 30 --stratified
...
python tools/speaker/03_eval.py \
  ...
  --asr-model-dir tools/asr/demo-model/zipformer_L_zh_en \
  --speaker-model tools/speaker/models/3dspeaker_speech_eres2net_base_sv_zh-cn_3dspeaker_16k.onnx \
  --out tools/speaker/results/eval_full.jsonl \
  --resume

# 汇总
python tools/speaker/04_eval_summary.py \
  --jsonl tools/speaker/results/eval_full.jsonl \
  --out-json tools/speaker/results/eval_full_summary.json \
  --out-md   tools/speaker/results/eval_full_summary.md
```
