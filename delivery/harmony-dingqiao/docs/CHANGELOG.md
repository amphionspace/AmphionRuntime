...
## 0.3.8 - 2026-08-24（机主识别稳定性与完整诊断交付）

- 修复 Speaker VAD 换人路径与 ASR `decodeAsync` 并发读取同一 native stream 的竞态；说话人推理
  使用独立异步通道，并通过不可变 timestamp 快照解析机主到他人的边界，避免交替讲话时应用闪退。
- 增加编译期 `diagnostics` 构建：完整记录 SDK 输入 PCM、结构化回调、有效配置、Runtime 指标和
  崩溃恢复 journal；普通 `debug` / `release` 构建仍关闭诊断采集。
- 保持公共 ASR、声纹、Speaker VAD 与 `isFinal` / `isLast` / `onComplete` 接口契约不变。
