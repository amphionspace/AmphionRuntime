...
## 0.3.7 - 2026-08-20（人名热词离线纠正）

- 新增 LAC-ONNX 人名识别与带声调的逐字拼音匹配，仅处理最终识别结果，不改变中间结果、声学模型
  或 beam search 路径。自定义热词中仅 2～3 字候选可进入匹配，不再因生僻人名被 LAC 单独误判为
  普通短语而提前丢弃。
- 同音签名只有一个已配置候选时才允许替换；二字候选仍要求与 LAC `PER` 人名区间重叠，三字
  候选允许在 LAC 漏标时使用完全同音兜底。四字及以上热词仍用于解码，但不进入 LAC final 替换。
  真机保存 PCM 已验证“给文富成发短信”和
  “该文富城发短信”均纠正为配置热词“文赋成”。
- 保持既有警务增强、声纹、VAD、partial/final 和 `isLast -> onComplete` 生命周期契约不变。
  目标说话人增强仍仅保留接口预留；本版本不包含该能力所需模型，不能启用该参数。

## 0.3.6 - 2026-08-20（警务增强与数字识别恢复）

- 默认中英识别模型升级为同时包含警务术语增强和数字识别恢复的版本。数字专项门禁使用客户在
  0.3.6 候选版本上现场确认的 10.88 秒 PTT 语音，并以同一 PCM、20ms 实时重放结果一致、18 位
  结构及校验码有效为通过条件；不再使用缺少人工真值的旧 6.62 秒语音阻断交付。
- 保留 0.3.5 的警务热词裁剪配置，并与新的警务增强声学模型配套交付；既有 Harmony 警务文本增强、
  中文热词 UTF-8 长度修复和公共 API 保持不变。
