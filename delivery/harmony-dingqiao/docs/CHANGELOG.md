...
## 0.3.17 - 2026-09-18（客户会议身份改善与句级角色输出）

- 角色定稿按完整原 ASR 句子输出；句内多人、重叠或未解决的未知不强归单一人物。`secondarySpeakerIndexes` 可记录句内人物，不能单凭非空判定重叠，迁移见 `UPGRADE_0.3.17.md`。
- 改善已确认客户录音中的同人重复建档、早期声音漏建档及部分主讲漏标；增加端侧 CAMPPlus 互补声纹证据，恢复客户离线录音中的角色4。MOSS仅用于研发标注，不进入SDK。
- 保留原始角色时间线、有限 UNKNOWN 推断标记和已提交窗口冻结；ASR final/last/complete 生命周期不变。本轮按客户录音收益交付，剩余误认、未知及性能代价如实保留。
- 与 Android 0.3.8 绑定同一构建源码提交。本版不包含 TTS 模型；目标说话人增强仅预留接口，不包含所需模型，不能启用。

## 0.3.16 - 2026-09-17（Android／鸿蒙联合交付）

- 与 Android 0.3.7 绑定同一最终源码提交重新构建；鸿蒙识别、身份算法及公开回调语义沿用 0.3.15。
- Android 修正时间舍入并对齐鸿蒙的原句展示与角色详情；本版不宣称新增鸿蒙身份精度修复。
- 已知短插话、重叠与部分身份误认继续阻断通用角色能力验收。详见 `UPGRADE_0.3.16.md`。
- 目标说话人增强仅预留接口；本交付不包含所需模型，不能启用。

## 0.3.15 - 2026-09-17（主讲人身份与文字可读性）

- 相对 0.3.14，更新主讲人相对音量准入和有真实取样支持的最终身份匹配，修复已验证录音中的主要发言串人；默认角色上限仍为 4。
- 保留完整词语和真实换人；同句有限 UNKNOWN 补全显式返回 `speakerInferred=true`、`confidence=0`，原始声学时间线不变。
- Diagnostics 增加最终窗口 UNKNOWN 原因记录，不增加公共接口字段，不改变身份规则。
- 生命周期和多批角色回调沿用上一版。短插话、轻声和重叠身份精度仍有限制，通用角色精度验收未通过；范围及迁移见 `UPGRADE_0.3.15.md`。
- 目标说话人增强仅预留接口；本交付不包含所需模型，不能启用。

## 0.3.14 - 2026-09-16（角色稳定性与双端 Demo 对齐）

- 修复同人短片段重复建档、历史混合上下文抢占身份和重叠通道证据丢失；不修改默认 4 人上限，不宣称多人精度问题已经解决。
- 修复角色分窗文本的标点/token 边界处理；中间角色可修正，已提交窗口保持冻结。
- 修复 Speaker VAD 后台角色处理延迟语音结束事件，以及初始等待、冷启动回灌和 Runtime 结束回收的边界问题。
- ITN 保留口语填充词；延续受控客户授权支持。
- Demo 明确角色中间/最终阶段、保留 SDK 编号，支持文件输入；会议 vadEnd 保持 800ms。
- 0.3.13 已有的分窗多批回调继续沿用，接入及已知限制见 `UPGRADE_0.3.14.md`。
- 目标说话人增强仅预留接口；本交付不包含所需模型，不能启用。
