...
## 0.3.6 - 2026-08-20（识别模型紧急回滚）

- 默认中英识别模型回滚到 0.3.4 已交付并验证过的模型版本。0.3.5 模型在客户 App 留存的
  6.62 秒身份证号语音中，native 解码阶段只输出“我的身份证号是”，未产生后续数字 token；
  本版本将同一语音的数字输出作为交付硬门禁。
- 撤回与 0.3.5 模型配套的警务热词裁剪配置，避免在声学模型回滚后继续使用不匹配的增强策略；
  既有 Harmony 警务文本增强、中文热词 UTF-8 长度修复和公共 API 保持不变。
