...
## 0.3.7 - 2026-08-21（amphion-119 与初始静音结果收口）

- 默认中英 ASR 替换为 `amphion-119-edge-transducer-chunk32-lc256-20260820-7.0.0-cp.5500`，
  并保留 0.3.6 产品链路的警务热词与文本增强。
- `vadBegin` 已确认超时后不再发布 native flush 产生的迟到推测 token；纯静音和稳态
  高能非语音仍只产生唯一空 last/complete，真实语音、手动 `finish` 和 endpoint 语义不变。
- Demo 页面改为纵向可滚动，小屏或大字体下底部操作不再因向下溢出而无法触达。
