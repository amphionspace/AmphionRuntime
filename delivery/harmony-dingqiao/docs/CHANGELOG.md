...
## 0.3.17 - 2026-09-24（双端 ASR SDK 更新）

- 中英识别模型升级到 Police 1.4.0，上游 encoder INT8、decoder/joiner FP32；两端同步警务词典与受限纠错。具体模型身份以最终 ZIP 的 manifest 和构建溯源为准。
- 修正活动 VAD endpoint 的待处理语音边界，减少已接受音频在句末被遗漏的风险。
- 更新会议角色的主要发言身份和句内可读性处理；HarmonyOS 缩短角色收尾待处理积压。已提交窗口继续冻结，临时角色仍可修正。
- 公开 `isFinal`、`isLast`、角色 `isSessionFinal` 与 `onComplete` 的含义保持不变。通用短插话、重叠身份精度不能凭本次改动宣称通过；详见 `UPGRADE_0.3.17.md`。
