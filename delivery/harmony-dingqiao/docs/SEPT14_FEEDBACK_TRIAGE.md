...
- **测试输入计时失真（已修复并验证）：** `feedPcmFile` 每帧工作后额外等待 20 ms，累计放慢输入，可能掩盖吞吐不足。旧逻辑在每帧工作 4 ms 时输入间隔变成 24 ms；现按起始时间和帧序号等待，不丢帧、不重排。两项可控时钟测试分别覆盖持续开销和单帧迟到；设备记录也确认约 150 秒提交 150 秒音频。此前分窗 PASS 仍可证明回调契约，不能作为持续实时吞吐的证据。
- **Harmony 推理超时原因分类错误（已独立修复）：** 本地执行器抛出 `timed out`，分类器只匹配 `timeout`，实际会归入 `INFERENCE_UNAVAILABLE`。后台对照已从公共分窗结果读到这一错误分类及原始超时文字。现使用本次任务自己的超时标记；回归测试在旧版得到原因 1，修复后为原因 3，同时验证迟到推理不会再次发布结果、cancel 后不报告超时。这不是附件中的 `FINISH_TIMEOUT` 根因；Android 已直接报告超时枚举，不走该字符串分支。
- **Harmony RTF 指标漏计（已确认，未修复）：** `drainAsync` 的耗时只随当前结果传给 final，`deliverSpeakerFinal` 用这一次耗时除以整句音频时长，未累计此前的流式解码。对照中累计原生解码约 105.5 秒，SESSION 仍显示 `avgRtf=0.000`。原始反馈中的 0.050 不能用于排除推理吞吐问题；该值也不包含输入排队时间。此问题不改变识别结果，但会误导性能诊断，须独立修复指标统计。
- **核心 VAD 大块混合输入漏掉尾静音（独立问题，未修复）：** 生产 gate 对同一调用中的多个 VAD 窗口聚合 `anySpeech`，只要包含语音就清零尾静音。固定检测状态为一个语音窗口加 25 个静音窗口（800 ms），512 样本分块产生 begin/end，合成一个大块只产生 begin；红灯保存在 `vad-mixed-block-diagnostic.log`。Android 有同名聚合逻辑，但尚未运行其动态对照。鼎桥公共接口要求每次 640 字节，因此不把核心大块输入问题归为本次客户起音漏事件根因，也不宣称现有尾静音修复覆盖所有混合分块。
- **过时的能力说明（待核对）：** AGENTS.md 的离线能力段仍把角色分离描述为尚未完成的服务方案，与 0.3.12 起的离线实现不一致。暂保留离线交付约束，不能据过时文字否定现有端侧实现，也不能据局部测试宣称全部交付门禁已完成。
- **文档提交阻止自动复用构建（工具限制，未改规则）：** 构建身份校验比较完整提交号，只有排查文档发生变化也会报过期。结束检查因此在原构建提交 `7d1f49f3` 上运行，结束后回到工作分支；未改校验代码、伪造构建身份或再次构建。文档提交与构建输入相同的证据仍保留。

原始计数和派生音频时间线保存在本轮诊断目录的 `customer-audio-progress.json`，不包含识别文本。

### 后台对照结果与边界

同一 PSN-AL00、诊断 HAP、150.477 秒 PCM 和 20 ms 绝对时间喂入，约第 30 秒切后台、第 60 秒回前台。每组相对默认配置只改变一个参数；后台已有载体申请的 AUDIO_RECORDING 连续任务。该载体喂入文件，没有实际麦克风采集，不能替代客户录音进程的完整后台环境。

| 配置 | 分窗回调检查 | 已观测最大积压 | 解释 |
| --- | --- | --- | --- |
| 4 线程、默认调度、允许忙等 | FAIL：推理超时降级 | 25.340 秒 | 建立后台红灯，并采集线程状态和原生 CPU 调用栈 |
| 仅关闭 ASR 忙等 | PASS，无降级 | 16.088 秒 | 空转热点消失，但仍跟不上后台实时输入；不是完整修复 |
| 仅指定 user-initiated 调度 | FAIL：推理超时降级 | 20.540 秒 | 调度请求实际生效，但单独调整优先级不解决问题 |
| 仅改为 1 个 ASR 线程 | PASS，无降级 | 20.822 秒 | 排除“只要减少线程数就能消除积压”的判断 |

这些差值来自每 250 帧的内部采样，属于已观测值，不是精确峰值。各组最终 ASR 文本和音频时间戳完全相同，均保留 finish 前 last=0、结束后唯一 last/complete。分窗 PASS 并未检查后台持续实时吞吐，不能混为一个结论。

默认组切后台后，提交音频继续，原生解码调用长期不返回；回前台后恢复。新增计时显示延迟主要在 native decode 调用内部，而不是无工作量的异步任务或 JS 回调返回。线程组由 `top-app` 变为 `background`；线程仍消耗 CPU，没有整进程冻结。CPU 采样显示 ONNX 任务队列轮询占用与系统工作线程竞争，关闭忙等后这一热点消失，但模型计算仍然变慢。因此已确认后台执行环境与吞吐下降的因果关联，尚未证明一个能在持续后台条件下排空输入的 SDK 调度修复；保留原默认配置，不以增加超时、强制绑核或提高优先级掩盖问题。

诊断代码以 `cf6d3bc5` 加临时计时插桩构建，HAP、补丁、构建身份、各轮失败现场、线程状态和采样数据均保留在本轮诊断目录。`backlog-comparison.json` 绑定四组报告；所有诊断轮次均为 non-canonical，不是发布验收。临时插桩已撤除，`7d1f49f3` 的原因分类修复不在这些旧诊断二进制中。
...
- `e419d2e6`：AGENTS.md 要求及时暴露附属问题并记录证据、影响和处理状态。
- `cf6d3bc5`：测试输入按绝对时间计时，避免逐帧处理开销拖慢输入。
- `7d1f49f3`：Harmony 本地推理超时原因使用任务内标记，保留 cancel 和迟到推理的资源回收边界。
- 继续排查后的 31 项相关主机测试通过；撤除临时插桩后，中英 Debug HAP 与四个 HAR 编译、签名及安装检查通过，代码提交 `7d1f49f3`，构建 fingerprint 为 `70af11a2ec9c4597b913500f486f07b30d28e82b3ab9db094a751c6ce7afb7bb`。
- 同一 `7d1f49f3` HAP 在 PSN-AL00 上通过结束兼容性检查：回调内 finish 3 轮、finish/shutdown 3 轮，SPEECH_END 重入返回非空唯一 last，随后唯一 complete；native stream 归零。根报告为 `followup-finish-compat/20260914-035008-668868-7d1f49f3/report.json`。短轮资源指标为 INCONCLUSIVE；这些用例不验收后台持续吞吐或推理超时的真机原因码。
