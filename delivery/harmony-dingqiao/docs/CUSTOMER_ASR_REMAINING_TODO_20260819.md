...
| 角色分离离线化 | **未关闭，不属于当前可交付能力** | 正式 SDK 要求完全离线。现有 `SpeakerDiarizationConfig.serviceUrl` 服务化路径不符合交付要求，不得要求业务配置 URL，也不得在 Demo、客户邮件或发布材料中宣称角色分离已交付。后续需改为 SDK 内端侧推理，移除网络与 PCM 上传依赖，并在断网条件下完成公共 API、结果和生命周期验收。 |
...
| 0.3.5 模型身份证数字缺失 | **通过模型回退规避** | 0.3.6 最终交付恢复为 0.3.4 已使用的稳定模型，不再交付发生数字缺失的 0.3.5 候选模型。此前新模型数字恢复实验及其证据不再代表最终交付。 |
| 最终旧模型包的发布基线 | **仅完成构建与打包检查** | 最终 ZIP 已绑定旧模型 source commit 和构建 provenance；按交付决定未重复执行真机发布矩阵。原 0.3.6 新模型证据保留为 superseded/non-canonical，不得用于证明最终旧模型二进制。 |
