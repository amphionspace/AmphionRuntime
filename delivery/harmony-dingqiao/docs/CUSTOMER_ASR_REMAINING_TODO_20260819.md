...
| 0.3.5 模型身份证数字缺失 | **0.3.6 回滚门禁已通过** | 客户 App 留存的 6.62 秒 PCM 在 0.3.5 native 解码阶段未产生身份证数字 token，已排除 UI 和字符串后处理。0.3.6 回滚到 0.3.4 已交付模型后，同一 PCM 已恢复完整数字串及校验位，native final 从 7 字符/8 token 恢复为 25 字符/25 token；该结论仅覆盖本次固定语料，不替代数字类准确率评测。 |
| 回滚模型的 AGC 基线 | **0.3.6 发布门禁通过** | 0.3.6 恢复旧 encoder `0e86…`，此前绑定该模型的 AGC 证据重新适用；本次同一 HAP 的 24 项 Harmony 门禁、63.25 秒用户序列和 Android Debug/Release 200 项测试均通过，不沿用 0.3.5 新模型 `ea36…` 的结论。 |
