...
2. **recognizerMode（short/long）**：两种模式使用同一流式模型，但 endpoint 语义不同。`short` 使用 `endpointMaxUtteranceMs` 作为单句硬上限；`long` 不做周期性 Rule3 硬切，只由自然静音或显式 `finish` 分段。`StartParams.extraParams` 可覆盖 engine 缺省模式；`vadBegin` 在两种模式下均生效。
