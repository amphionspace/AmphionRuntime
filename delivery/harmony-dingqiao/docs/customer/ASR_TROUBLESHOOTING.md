...
严格筛选后的有效语音达到 `minSegSec` 时优先使用严格样本。严格样本不足，但 ASR 已产生非空
text/token 且当前句实际 PCM 达到门槛时，SDK 使用当前句真实 PCM 回退评分。没有 ASR 语音证据或
实际 PCM 仍短于门槛时省略分数并保留识别结果。相似度精度必须使用带身份标注的目标/非目标语料
评测，不能只用生命周期日志判断。
...
确认授权文件可读、未被修改、授权能力包含 ASR、设备标识类型与签发清单一致、SDK 主版本与维护期有效。完整说明见 [LICENSE.md](LICENSE.md)。
