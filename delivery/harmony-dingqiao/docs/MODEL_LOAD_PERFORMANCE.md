...
在 HarmonyOS 6.1 arm64 真机 A（型号与设备标识已脱敏）、相同系统构建和模型源文件下：
...
### 2026-07-12 第二台真机单变量复核

在另一台 HarmonyOS 6.1 arm64 真机 B（型号与设备标识已脱敏，处理器代际比真机 A 新一代）
上做**单变量对照**：只把模型格式在 ORT 与 ONNX 之间切换
...
绝对值随设备/系统/热状态而变，只在 comparison identity 一致时可比；此第二台真机单次冷启结论
与真机 A 的 80.1% 同向印证（真机 A 的 3884.5 ms 是较早硬件上的全串行优化前路径）。
