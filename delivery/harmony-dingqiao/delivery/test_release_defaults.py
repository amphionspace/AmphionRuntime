...
class ReleaseDefaultsTest(unittest.TestCase):
    def test_0313_versions_and_upgrade_contract_are_consistent(self) -> None:
        changelog = (
            REPO_ROOT / "delivery/harmony-dingqiao/docs/CHANGELOG.md"
        ).read_text(encoding="utf-8")
        notes_0313 = changelog.split("## 0.3.13", 1)[1].split("\n## ", 1)[0]
        upgrade = (
            REPO_ROOT / "delivery/harmony-dingqiao/docs/customer/UPGRADE_0.3.13.md"
        ).read_text(encoding="utf-8")

        self.assertIn("冷启动", notes_0313)
        ...
        version_files = {
            "asr/harmony/sdk/oh-package.json5": '"version": "0.3.13"',
        ...
        self.assertIn("UPGRADE_0.3.13.md", pack_script)
        validator = (
            REPO_ROOT
    ...
    def test_android_prepares_isolated_sherpa_before_cache_or_bridge_checks(self) -> None:
        workflow = (REPO_ROOT / ".github/workflows/android.yml").read_text(encoding="utf-8")
        android = workflow.split("  android-aar:", 1)[1].split("  ci-result:", 1)[0]

        prepare_source = android.index("bash asr/tools/prepare_sherpa_source.sh")
        self.assertLess(prepare_source, android.index("Compute Android native source fingerprint"))
        self.assertLess(prepare_source, android.index("Verify Kotlin bridge in sync with submodule"))
        patch_step = android.split("- name: Prepare isolated patched sherpa-onnx source", 1)[1].split(
    ...
    def test_0312_docs_do_not_claim_speaker_vad_blocks_cold_start(self) -> None:
        ...
        for path in doc_paths:
            ...
            for claim in stale_claims:
                self.assertNotIn(claim, text, f"stale 0.3.13 behavior in {path}: {claim}")
