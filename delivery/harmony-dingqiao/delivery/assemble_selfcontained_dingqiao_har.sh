#!/usr/bin/env bash
# 组装"自包含" amphion_dingqiao.har:把 amphion_asr / sherpa_onnx 打进包内,
...
# 依赖: 三个 HAR 已由 DevEco 构建(见各模块 build/default/outputs/default/)。
set -euo pipefail
...
for h in "$ASR_HAR" "$DINGQIAO_HAR" "$SHERPA_HAR"; do
