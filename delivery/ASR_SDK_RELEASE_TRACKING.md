...
每条记录对应一个已经通过终检的最终 ZIP，保存平台版本、构建源码 commit、交付日期、
附件名、附件 SHA-256/字节数和包内 provenance 哈希。`record` 直接从最终 ZIP 读取 provenance，
...
python3 tools/delivery/asr_release_tracker.py record \
  ...
  --artifact /path/to/amphion-dingqiao-asr-sdk-v0.3.3-20260730.zip
...
python3 tools/delivery/asr_release_tracker.py record \
  ...
  --source-commit "$SOURCE_COMMIT" \
  --delivered-at 2026-07-30 \
  --artifact /path/to/amphion-harmony-asr-sdk-v0.2.9-20260730.zip
```

`record` 会校验 ZIP CRC，从包内唯一 provenance 验证版本和完整 commit，并记录 ZIP 本身的
