...
#   ./run_batch.sh all --hotword-profile=prune_ui28 # 真机回退筛除后的首批28项候选
#
...
for arg in "${@:2}"; do
  case "$arg" in
    ...
    --hotword-profile=prune_ui28) HOTWORD_PROFILE=prune_ui28 ;;
    --hotword-profile=none) HOTWORD_PROFILE=none ;;
    *) echo "[run] 未知参数 '$arg'（可选 --resume|--hotword-profile=full|--hotword-profile=prune_ui28|--hotword-profile=none）"; exit 1 ;;
  esac
done
