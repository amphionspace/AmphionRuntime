...
文本中的 LF、CRLF、CR 和 Unicode 换行/段落分隔符会按行末标点处理：行末没有标点时补 `.`；已有句号、问号、感叹号、逗号、分号、冒号、顿号或省略号时保留，不重复补点。引号/括号结尾时检查其前面的正文，在闭合符前补句号。连续空行不累计标点，普通空格不受影响。

句号使用当前符号表已有的 token，停顿由模型生成，不保证固定毫秒数。视觉上的自动折行不属于输入换行；从文档复制的硬换行会触发上述规则。
