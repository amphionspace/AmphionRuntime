...
def main() -> None:
    ...
    ap.add_argument("--device-sha256", default="", help="兼容旧 license 的宿主设备 hash")
    ...
    if bound_app != host_app:
        sys.exit(f"[FAIL {LICENSE_APP_MISMATCH}] LICENSE_APP_MISMATCH：license={bound_app} host={host_app}")
    ...
    if features and args.required_feature not in features:
    ...
    elif claims.get("deviceSha256", ""):
        have_dev = _norm_hex(args.device_sha256)
        if not have_dev:
            print("[warn] license 绑定了 deviceSha256，但未提供 --device-sha256，跳过设备校验")
        elif have_dev != _norm_hex(claims.get("deviceSha256", "")):
            sys.exit(f"[FAIL {LICENSE_DEVICE_MISMATCH}] LICENSE_DEVICE_MISMATCH：legacy device hash mismatch")
