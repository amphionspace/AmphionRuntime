...
private:
    ...
    icu::UnicodeString numberToEnglish(const icu::UnicodeString& numStr) {
    ...
    icu::UnicodeString numberToOrdinalEnglish(const icu::UnicodeString& numStr, UErrorCode& status) {
        if (U_FAILURE(status)) {
            return numStr;
        }
        ...
        // Integer path: more reliable than double for day-of-month ordinals (e.g. 22 → twenty-second).
